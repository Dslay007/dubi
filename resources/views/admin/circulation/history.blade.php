...
<div class="card" style="background: white; border-radius: 0.5rem; overflow: hidden; border: 1px solid #e2e8f0;">
    <div style="padding: 1.5rem; border-bottom: 1px solid #e2e8f0;">
         <h3 style="font-weight: 700; color: #1e293b;">Sejarah Peminjaman</h3>
    </div>
        ...
        <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 0.875rem;">
            <thead>
                <tr style="background: #f1f5f9; color: #475569; text-transform: uppercase; font-size: 0.75rem; font-weight: 700;">
                     ...
                     <th style="padding: 0.75rem 1.5rem; border-bottom: 1px solid #e2e8f0;">Status</th>
                     <th style="padding: 0.75rem 1.5rem; border-bottom: 1px solid #e2e8f0;">Return Date</th>
                </tr>
            </thead>
            <tbody>
                @forelse($loans as $loan)
                <tr style="border-bottom: 1px solid #e2e8f0;">
                    <td style="padding: 1rem 1.5rem; color: #64748b;">{{ $loan->loan_date }}</td>
                    ...
                    <td style="padding: 1rem 1.5rem;">
                        @if($loan->is_return)
                            <span style="background: #dcfce7; color: #166534; padding: 0.25rem 0.5rem; border-radius: 99px; font-size: 0.75rem; font-weight: 600;">Returned</span>
                        @else
                            <span style="background: #fee2e2; color: #b91c1c; padding: 0.25rem 0.5rem; border-radius: 99px; font-size: 0.75rem; font-weight: 600;">On Loan</span>
                        @endif
                    </td>
                    <td style="padding: 1rem 1.5rem; color: #64748b;">{{ $loan->return_date ?? '-' }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" style="padding: 2rem; text-align: center; color: #64748b;">No loan history found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
