...
@section('pageTitle', 'Import Penerbit (CSV)')

@section('content')
<div style="background: white; border-radius: 0.5rem; border: 1px solid #e2e8f0; padding: 2rem; max-width: 800px;">
    <h3 style="font-weight: 700; color: #1e293b; margin-bottom: 1rem;">Impor Data Penerbit dari CSV</h3>
    ...
    <div style="background: #f1f5f9; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1.5rem; font-size: 0.9rem; color: #475569;">
        ...
        <p>Susunan kolom pada baris pertama (Header) dilewati. Pastikan susunan data dari kolom 1 s/d 2 seperti di bawah ini:</p>
        <ul style="margin-left: 1.5rem; margin-top: 0.5rem;">
            <li>Kolom 1: <strong>ID Penerbit</strong> (Kosongkan jika ingin ID otomatis baru)</li>
            <li>Kolom 2: <strong>Nama Penerbit</strong> (Wajib)</li>
        </ul>
    </div>

    <form action="{{ route('admin.publisher.process_import') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div style="display: flex; gap: 1rem;">
            ...
            <a href="{{ route('admin.publisher.index') }}" class="btn" style="background: #f1f5f9; color: #475569; padding: 0.5rem 1.5rem; text-decoration: none; border-radius: 0.375rem;">Batal</a>
        </div>
    </form>
</div>
@endsection
